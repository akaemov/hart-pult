...
///
/// Параллельные запуски одного источника здесь не предотвращаются: на сервере
/// сбор запускают systemd-таймеры, а таймер не стартует службу, пока её
/// предыдущий запуск не завершился.
...
export async function runSync(source: SourceId, collect: Collector) {
