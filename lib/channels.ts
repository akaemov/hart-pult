...
export const OBJECTS = ["Заря", "Пьермонт", "Не определён"] as const;
...
/// Объект определяется по трём признакам подряд: название сделки, лендинг,
/// рекламная кампания. Поле «ЖК» в amoCRM заполнено у 3% сделок и потому
/// бесполезно. Если сделка помечена обоими объектами сразу — это не ответ,
/// а неопределённость.
